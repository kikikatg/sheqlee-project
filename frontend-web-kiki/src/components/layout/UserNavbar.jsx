...
const UserNavbar = () => {
  ...
  useEffect(() => {
    if (profileOpen && profileTriggerRef.current) {
      ...
      setDropdownPos({
        top: rect.bottom, // directly under Microsoft
        right: window.innerWidth - rect.right - 12, // push right slightly
      });
    }
  }, [profileOpen]);
        ...
        <nav className="w-full max-w-[1920px] mx-auto flex items-center h-full px-4 md:px-[101px]">
          ...
          <div className="ml-auto hidden md:flex items-center gap-12">
            ...
            >
              ...
              <span className="text-[22px] font-medium">Microsoft</span>
...
>
  ...
  Dashboard
          ...
          <Link
          to="/account-setting" 
           onClick={() =>setProfileOpen(false)}className="flex items-center gap-4 px-6 py-3 hover:bg-[#F4F1FA]">
